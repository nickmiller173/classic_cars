...
This repo contains an independent machine learning platform built entirely on Cars & Bids auction data. The platform combines a predictive pricing engine with market analysis across ~31,000 auctions and ~$883M in recorded sales. It offering buyers, sellers, and enthusiasts an objective lens on what cars are actually worth.
...
processing/preprocessing.ipynb     ← cleans data, engineers features, saves train/test splits
      │
      ├──▶ data/frontend_data/      ← CSVs consumed directly by the Streamlit frontend, created by separate .ipynb files
      │
      ▼
...
| `train_data.csv` / `test_data.csv` | **Gitignored.** The actual train/test data that is used for model building. |
...
| `cleaned_data_no_encoding.csv` | `preprocessing.ipynb` | Home page stats | **Gitignored** (194MB). The full pre-ML dataset without label encoding — used for accurate aggregate stats (GMV, median price). Falls back to `dashboard_data.csv` if missing. |
...
- **SHAP importance:** Bar length = mean absolute SHAP value = average impact magnitude regardless of direction. A feature that alternately adds and subtracts $5K still scores high. The 20 individual `text_component_N` SVD dimensions are collapsed into a single **"Listing Description Text"** bar (their SHAP values are summed) so the chart remains readable to non-technical users.
